...
Template fixture cells:

| Sheet | Cell | Value |
|---|---|---|
| Sheet1 | B2 | `{{ revenue }}` |
| Sheet1 | C4 | `{{ title \| orientation=horizontal }}` |
| Sheet1 | D6 | `{{ count \| skip=2, flag=True }}` |
| Sheet1 | E8 | `{{ items \| loop() }}` |
| Sheet1 | E9 | `{{ items \| loop() }}` ← second loop cell same row |
| Sheet2 | A1 | `{{ summary }}` |
...
## ExcelTemplateReader Test Cases

### Happy path

```python
def test_returns_worksheets_with_tags(template_path):
...
```python
def test_marked_cell_attributes(template_path):
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    cell = next(c for c in result['Sheet1'] if c.name == 'revenue')
...
```python
def test_tag_with_metadata_parsed(template_path):
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    cell = next(c for c in result['Sheet1'] if c.name == 'title')
    assert cell.metadata == 'orientation=horizontal'
    assert cell.parse_metadata() == {'orientation': 'horizontal'}
```

```python
def test_function_call_metadata_type(template_path):
    """loop() tag parses to {"type": "loop"}."""
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    cell = next(c for c in result['Sheet1'] if c.name == 'items')
    assert cell.parse_metadata() == {'type': 'loop'}
```

```python
def test_function_call_metadata_with_options():
    mc = MarkedCell(
        name='sector_table', sheet='S', cell_addr='B5',
        raw='{{ sector_table | table(join=outer, on=Sector) }}',
        metadata='table(join=outer, on=Sector)',
    )
    assert mc.parse_metadata() == {'type': 'table', 'join': 'outer', 'on': 'Sector'}
```

```python
def test_metadata_type_coercion(template_path):
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    cell = next(c for c in result['Sheet1'] if c.name == 'count')
    meta = cell.parse_metadata()
    assert meta['skip'] == 2           # int
    assert meta['flag'] is True        # bool
```

```python
def test_sheets_without_tags_excluded(template_path):
    """Sheets with no {{ }} tags must not appear in the result."""
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
...
```python
def test_no_metadata_returns_empty_dict(template_path):
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    cell = next(c for c in result['Sheet1'] if c.name == 'revenue')
...
```python
def test_empty_tag_name_raises(tmp_path):
    """A cell containing {{ }} (no name) must raise TemplateReadError."""
    wb = Workbook()
    ws = wb.active
    ws['A1'] = '{{ }}'
...
```python
def test_metadata_missing_equals_raises(template_path):
    """parse_metadata raises when a fragment has no '=' separator."""
    reader = ExcelTemplateReader()
    result = reader.read(template_path)
    bad = MarkedCell(name='x', sheet='S', cell_addr='A1', raw='{{ x | badkey }}', metadata='badkey')
    with pytest.raises(TemplateReadError):
        bad.parse_metadata()
```
